...
export default function PrecificacaoAdvogados() {
  const { t } = useTranslation();

  return (
    <>
      <SEO page="coursePrecificacao" path="/training/precificacao-advogados" />
          ...
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <AnimatedSection className="prose-container">
              ...
              <p className="text-lg text-gray-700 leading-relaxed mb-8">
                {t("courses.precificacao.intro")}
              </p>

              {/* Target Audience */}
              <h2 className="text-2xl font-semibold text-[#034633] mb-4 mt-12">
                {t("courses.precificacao.targetTitle")}
              </h2>
              <ul className="space-y-3 mb-8">
                {(t("courses.precificacao.targetList", { returnObjects: true }) as string[]).map(
                  (item, i) => (
              ...
              {/* Program */}
              <h2 className="text-2xl font-semibold text-[#034633] mb-4 mt-12">
                {t("courses.precificacao.programTitle")}
              </h2>
              <div className="space-y-4 mb-8">
                {(t("courses.precificacao.modules", { returnObjects: true }) as Array<{ title: string; items: string[] }>).map(
                  (mod, i) => (
              ...
              {/* Methodology */}
              <h2 className="text-2xl font-semibold text-[#034633] mb-4 mt-12">
                {t("courses.precificacao.methodologyTitle")}
              </h2>
              <ul className="space-y-3 mb-8">
                {(t("courses.precificacao.methodologyList", { returnObjects: true }) as string[]).map(
                  (item, i) => (
                    <li key={i} className="flex items-start gap-3">
              ...
              {/* Results */}
              <h2 className="text-2xl font-semibold text-[#034633] mb-4 mt-12">
                {t("courses.precificacao.resultsTitle")}
              </h2>
              <ul className="space-y-3 mb-8">
                {(t("courses.precificacao.resultsList", { returnObjects: true }) as string[]).map(
                  (item, i) => (
                    <li key={i} className="flex items-start gap-3">
              ...
              <div className="bg-[#034633] rounded-2xl p-8 my-12">
                <h2 className="text-2xl font-semibold text-white mb-4">
                  {t("courses.precificacao.formatTitle")}
                </h2>
                <ul className="space-y-3">
                  {(t("courses.precificacao.formatList", { returnObjects: true }) as string[]).map(
                    (item, i) => (
                      <li key={i} className="flex items-start gap-3">
              ...
              <div className="text-center mt-12 pt-8 border-t border-gray-200">
                <h2 className="text-2xl font-semibold text-[#034633] mb-4">
                  {t("courses.precificacao.ctaTitle")}
                </h2>
                <p className="text-gray-600 mb-6">
                  {t("courses.precificacao.ctaText")}
                </p>
                <Link
                  to="/contact"
